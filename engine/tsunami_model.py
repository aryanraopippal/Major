...
def run_tsunami(params, db_conn, financial_model=None):
    ...
    wave_height_m = sanitize_float(params.get("mag"), 1.0)
    ...
    try:
        ...
        for table, key, radius, default_vuln in tables:
            ...
            for name, t_lat, t_lon, dist in cur.fetchall():
                ...
                impact = max(0.0, min(10.0, impact + random.uniform(-0.05, 0.05)))
    ...
    macro_recovery_years = xgb_model.predict([[wave_height_m, max_physical_distance_km, 7.0]])[0] if ai_active else wave_height_m / 2.0
